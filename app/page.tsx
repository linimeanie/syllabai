...
const NICHES = [
  ...
  "Radio certificate (SRC/UBI)",
  "Hunting licence (Jagdschein)",
  OTHER,
];
...
export default function Home() {
  ...
  async function generate() {
    ...
    setError(null);
    try {
      const effectiveNiche = niche === OTHER ? customNiche.trim() : niche;
      const form = new FormData();
      form.append("text", text);
      form.append("niche", effectiveNiche);
          ...
          <>
            ...
            <div className="mt-10 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
              ...
              <select
                ...
                className={`w-full rounded-lg border border-stone-300 px-3 py-2.5 text-sm bg-white ${
                  niche === OTHER ? "mb-2" : "mb-5"
                }`}
              >
                {NICHES.map((n) => (
              ...
              {niche === OTHER && (
                <input
                  ...
                  placeholder="e.g. Forklift licence, Drone pilot A2, Tax-advisor exam…"
                  className="w-full rounded-lg border border-stone-300 px-3 py-2.5 text-sm mb-5"
                  autoFocus
                />
              )}
